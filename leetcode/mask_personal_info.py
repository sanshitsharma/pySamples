...
To mask an email, all names must be converted to lowercase and all letters between the first and last letter of the first name must
...
Note that extraneous characters like "(", ")", " ", as well as extra dashes or plus signs not part of the above formatting scheme
should be removed.
...
Ref: https://leetcode.com/problems/masking-personal-information/description/ 
'''
...
class Solution(object):
    def maskPII(self, S):
        ...
        if S[0] == '+' or S[0].isdigit() or S[0] == '(':
            ...
            #print "Masking an email address"
            res = S[0].lower() + '*****'
            i = 1
            while S[i+1] != '@':
                i += 1
            
            res += S[i].lower() + S[i+1:len(S)].lower()

        return res

if __name__ == "__main__":
    inputs = ["LeetCode@leetCode.com", "AB@qq.com", "1(234)567-890", "86-(10)12345678", "+46(427)55-7-41", "+(501321)-50-23431"]
    for info in inputs:
        print Solution().maskPII(info)